...
pub fn build_installation_tab(cx: &mut Context) {
    Binding::new_fallible(
        ...
        AppState::config
            ...
            .then(UnwrapLens::new()),
        |cx, root| {
            Label::new(cx, &format!("Current celeste install is {:?}", root.get(cx)));
            Binding::new(cx, AppState::modules_version, move |cx, _| {
                ...
                for (sid, num_maps, name) in modules_list.into_iter() {
                    VStack::new(cx, move |cx| {
                        Label::new(cx, &name)
                            .class("module_title");
                        Label::new(cx, &format!("{} map{}", num_maps, if num_maps == 1 { "" } else { "s" }));
                    })
                        .class("module_overview_card")
                        .on_press(move |cx| {
                            cx.emit(AppEvent::OpenModuleOverview { module: sid.clone() })
                        });
                }
            });
        }, |cx| {
            Button::new(cx, move |cx| {
                if let Ok(Some(celeste_path)) = dialog::FileSelection::new("Celeste Installation")
                        .title("Please choose Celeste.exe")
                        .path(".")
                        .mode(dialog::FileSelectionMode::Open)
                        .show() {
                    cx.emit(AppEvent::SetConfigPath { path: Path::new(&celeste_path).parent().unwrap().to_path_buf() });
                }
            }, move |cx| {
                Label::new(cx, "Select Celeste.exe")
            });
