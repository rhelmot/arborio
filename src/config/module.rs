...
impl CelesteModule {
    pub fn new<T: ConfigSource>(source: &mut T) -> Self {
        ...
        if let Some(fp) = source.get_file(&PathBuf::from("Graphics/ForegroundTiles.xml")) {
            tilers.insert(
                "fg".to_owned(),
                Tileset::load(fp, &gameplay_graphics, "tilesets/")
            ...
            if let Some(fp) = source.get_file(&path) {
                tilers.insert(
                    path.file_stem()
                        ...
                        .to_owned(),
                    Tileset::load(fp, &gameplay_graphics, "")
                        .expect("Could not parse custom tileset"),
                );
            }
        }

        let entity_config = source
            .list_all_files(&PathBuf::from("Arborio/entities"))
            .map(|f| {
                let mut config: EntityConfig =
                    serde_yaml::from_reader(source.get_file(&f).unwrap())
                        .expect("Failed to parse entity config");
            ...
            .collect();
        let trigger_config = source
            .list_all_files(&PathBuf::from("Arborio/triggers"))
            .map(|f| {
                let mut config: TriggerConfig =
                    serde_yaml::from_reader(source.get_file(&f).unwrap())
                        .expect("Failed to parse trigger config");
                if config.templates.is_empty() {
                    config.templates.push(config.default_template());
                }
