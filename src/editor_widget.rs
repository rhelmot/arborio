...
use crate::image_view::{ImageBuffer, ImageViewMut};
...
impl EditorWidget {
    ...
    fn draw(&mut self) {
        ...
        self.widget.draw(move |b| {
            ...
            if let Some(ref map) = state.map {
                ...
                for room_idx in 0..state.map.as_ref().unwrap().levels.len() {
                    ...
                    if rect_screen.intersects(&screen) {
                        ...
                        let should_draw_complex = state.transform.map_scale >= 8;
            ...
            // let blob = assets::GAMEPLAY_ATLAS.blobs[0].as_ref();
            // blob
            //     .resize(blob.width * state.map_scale / 8, blob.height * state.map_scale / 8)
            //     .draw_clipped(&screen, -state.map_corner_x, -state.map_corner_y);
            draw::pop_clip();
        ...
        for ty in 0..room.bounds.height / 8 {
            for tx in 0..room.bounds.width / 8 {
                ...
                if let Some(tile) = tiles_asset.get(&tile).and_then(|tileset| tileset.tile(room, foreground, tx as i32, ty as i32)) {
                    assets::GAMEPLAY_ATLAS.draw_tile(tile, tx * self.transform.map_scale, ty * self.transform.map_scale, self.transform.map_scale, room_buffer.reborrow(), resized_sprite_cache);
                }
            }
        }
