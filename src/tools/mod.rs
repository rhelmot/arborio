            ...
            if let Some(map) = state.loaded_maps.get(&state.map_tab_unwrap().id) {
                ...
                if let Some(idx) = map.level_at(map_pt) {
                    if idx != state.map_tab_unwrap().current_room {
                        return vec![AppEvent::SelectRoom { tab: state.current_tab, idx }];
                    }
                }
            }
            vec![]
